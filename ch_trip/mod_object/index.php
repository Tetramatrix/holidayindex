...
***************************************************************/
/**
 * Module 'chtripM1' for the 'ch_trip' extension.
 *
 * @author	Chi Hoang
 */
...
$LANG->includeLLFile("EXT:ch_trip/mod_object/locallang.php");
...
class tx_chtrip_object extends tx_chtrip_scriptclasses {
	...
	function main()	{
		...
		if (($this->id && $access) || ($BE_USER->user["admin"] && !$this->id))	{
			...
			if (is_array($CMD)) { 
				$uid = array_keys($CMD['SELECT']['txchvaObject']);
				$addParams = '?SLCMD[SELECT][txchvaObject]['.$uid[0].']=1';
			}

    				// JavaScript
			$this->doc->JScodeArray['redirectUrls'] = $this->doc->redirectUrls(t3lib_extMgm::extRelPath('ch_trip').'mod_object/index.php'.$addParams);
			...
			*/
			...
			$this->content.=$this->doc->startPage("Trip: Hotels");
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/ch_trip/mod_object/index.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/ch_trip/mod_object/index.php']);
}

// Make instance:
$SOBE = t3lib_div::makeInstance('tx_chtrip_object');
$SOBE->init();
