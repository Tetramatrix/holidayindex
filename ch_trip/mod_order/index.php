...
$LANG->includeLLFile("EXT:ch_trip/mod_order/locallang.php");
...
class tx_chtrip_order extends tx_chtrip_scriptclasses {
	...
	function main()	{
		...
		if (($this->id && $access) || ($BE_USER->user["admin"] && !$this->id))	{
			...
			if (is_array($CMD)) { 
				$uid = array_keys($CMD['SELECT']['txchvaOrder']);
				$addParams = '?SLCMD[SELECT][txchvaOrder]['.$uid[0].']=1';
			}

    				// JavaScript
			$this->doc->JScodeArray['redirectUrls'] = $this->doc->redirectUrls(t3lib_extMgm::extRelPath('ch_trip').'mod_order/index.php'.$addParams);
			...
			*/
			...
			$this->content.=$this->doc->startPage("Trip: Bestellungen");
			$this->content.=$this->doc->header($LANG->getLL("title"));
			$this->content.=$this->doc->spacer(5);
			$this->content.=$this->doc->header($LANG->getLL("order").$title);
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/ch_trip/mod_order/index.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/ch_trip/mod_order/index.php']);
}

// Make instance:
$SOBE = t3lib_div::makeInstance('tx_chtrip_order');
$SOBE->init();
