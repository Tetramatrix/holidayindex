...
***************************************************************/
/**
 * Module 'VacationindexM1' for the 'ch_trip' extension.
 *
...
$LANG->includeLLFile("EXT:ch_trip/mod_prop/locallang.php");
...
class tx_chtrip_prop extends tx_chtrip_scriptclasses {
	var $pageinfo;
	...
	function main()	{
		...
		if (($this->id && $access) || ($BE_USER->user["admin"] && !$this->id))	{
			...
			if (is_array($CMD)) {			
				$uid = array_keys($CMD['SELECT']['txchvaProp']);
				$addParams = '?SLCMD[SELECT][txchvaProp]['.$uid[0].']=1';
    		...
    		if (is_array($CMD)) { 
        		...
        		$addParams = '?SLCMD[SELECT][txchvaProp]['.$uid[0].']=1';
    				...
    				// JavaScript
			$this->doc->JScodeArray['redirectUrls'] = $this->doc->redirectUrls(t3lib_extMgm::extRelPath('ch_trip').'mod_prop/index.php'.$addParams);
			...
			*/
			...
			$this->content.=$this->doc->startPage("Trip: Eigenschaften");
			$this->content.=$this->doc->header($LANG->getLL("title"));
			$this->content.=$this->doc->spacer(5);
			$this->content.=$this->doc->header($LANG->getLL("property").$title);
	...
	function moduleContent()	{
    	...
    	if (is_array($CMD)) {
			...
			$uid = array_keys($CMD['SELECT']['txchvaProp']);
				...
				// get properties
			$this->tree = t3lib_div::makeInstance("t3lib_treeView");
			$this->tree->init();
			$this->tree->table = 'tx_chtrip_properties';
			$this->tree->parentField = 'parent_uid';
			$this->tree->treeName = 'tx_chtrip_properties';
			$this->tree->expandAll = 1;
			$tree = $this->tree->getTree($uid[0]);
			
			if($tree>0) {
				foreach($this->tree->tree as $k => $v) {
					$properties[]=$v['row']['uid'];	
				}				
			} else {
				$properties[] =  $uid[0];	
			}
			...
			$table='tx_chtrip_hotel,tx_chtrip_properties_mm,tx_chtrip_region,tx_chtrip_properties';
			$results_at_a_time = 20;
			
			$addWhere = ' AND tx_chtrip_hotel.uid=tx_chtrip_properties_mm.uid_local
					AND tx_chtrip_hotel.location_f04338f846=tx_chtrip_region.uid
					AND tx_chtrip_properties_mm.uid_foreign=tx_chtrip_properties.uid
					AND tx_chtrip_hotel.deleted=0 AND tx_chtrip_hotel.hidden=0
					AND tx_chtrip_properties_mm.uid_foreign IN ('.implode(',',$properties).')';

			if (t3lib_div::_GET('ter_pointer')) {
				$this->pb->ter_pointer = t3lib_div::_GET('ter_pointer');
			}

			$this->pb->pi_listFields =  'tx_chtrip_hotel.uid,
										tx_chtrip_hotel.title,
										tx_chtrip_hotel.location_f04338f846,
										tx_chtrip_hotel.teaser,
										tx_chtrip_hotel.pictures,
										tx_chtrip_hotel.location,
										tx_chtrip_hotel.tstamp,
										tx_chtrip_properties_mm.uid_local AS objTypeUidLocal,
										tx_chtrip_properties.uid AS typeUid,
										tx_chtrip_properties.parent_uid AS typeParent,
			...
			$this->pb->internal["currentTable"] = "tx_chtrip_hotel";
			$this->pb->internal['pagefloat'] = 'center';
	
				// Get number of records:
			$res = $this->pb->pi_exec_query($table,1,$addWhere,'','','','',2);
			list($this->pb->internal['res_count']) = $GLOBALS['TYPO3_DB']->sql_fetch_row($res);


				// Make listing query, pass query to SQL database:
			$res = $this->pb->pi_exec_query($table,0,$addWhere,'','','','',2);
			...
			if (sizeOf($lines)) {
				...
				foreach ($lines as $k => $v) {
					...
					$cmd = 'onMouseover=highlight(this,\'yellow\') onMouseout="highlight(this,document.bgColor)" onClick="'.$this->DB_edit('tx_chtrip_hotel',$v['uid']).'"';
					switch ($kill) {					
						case 1:
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/ch_trip/mod_prop/index.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/ch_trip/mod_prop/index.php']);
}

// Make instance:
$SOBE = t3lib_div::makeInstance('tx_chtrip_prop');
$SOBE->init();
